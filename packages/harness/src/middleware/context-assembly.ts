// Renders the system prompt: generic engine guidance + the agent's own
// instruction fragments + available skills + step guidance. Structured-output
// instructions are contributed separately by the structured-output middleware.
...
export const contextAssembly: MiddlewareFactory = () => ({
  ...
  contributeSystem(ctx) {
    const prompts: string[] = [BASE_SYSTEM_PROMPT, ...ctx.agent.instructions.filter(Boolean), TOOL_USE_SYSTEM_PROMPT]
