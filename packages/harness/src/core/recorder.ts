 ...
 * emitter of turn-scoped loop telemetry (turn.start / turn.phase / turn.retry /
 * turn.end). Content writes go through the Sessions aggregate, so state events
 * fall out of the writes themselves; the recorder never emits state by hand.
 *
...
export class TurnRecorder {
  ...
  /** Records a model-call retry and emits turn.retry. */
  recordRetry(retry: {
    attempt: number
    delayMs: number
    ...
    this.retryCount += 1
    this.input.loop.emit({ type: "turn.retry", ...this.envelope(), messageID: this.messageID, ...retry })
  }
