// A scriptable Model stub for tests: replays a fixed chunk script per stream call
// and (optionally) records each input, so a test can drive the loop deterministically
// without a network provider and assert which model received a call.
import type { LLMChunk, LLMInput, Model, ProviderModelSpec } from "@harness/llm/types"
