 ...
 * runs, and is the concurrency-safe unit a step fans out: it owns only its own
 * tool part (via the injected tracker) and never touches the step's terminal
 * state — collecting outcomes and deciding the step's continue/stop is the
 * caller's job (see step.ts). Business policy lives in middleware: budgets and
 ...
 * recoverable error (the step continues), a stop signal (a gate or afterToolCall
 * decided the step must halt), or an abort. The caller reduces a batch of these
 * into the step's single terminal decision.
 */
 ...
 *
 * @param recorder - the step recorder that owns part creation
 * @param call - the tool name, call id, and raw args
 * @param prepared - the result of prepareToolCall
 * @returns the tracker for this call's part
 */
export function openToolPart(recorder: StepRecorder, call: ToolCall, prepared: PreparedToolCall) {
 ...
 * tracker) and never touches the step's terminal state, so N calls run at once
 * and the caller decides the step's fate after they all settle.
 *
 * @param ctx - the step context
 * @param stack - the middleware stack (its afterToolCall settlement runs here)
 * @param recorder - the step recorder (used only to spawn nested tool parts)
 * @param call - the tool name, call id, and raw args
...
// truth, and a stop escalation halts the step.
...
function collectSessionHistory(ctx: StepContext): SessionHistoryMessage[] {
