...
一次性定义三样东西，让 harness 与各 surface 不再各持一份表示：
...
- harness 经源码别名 `@contracts` 引用；包外消费方按包名 `@agent-loop/contracts` 引用。
- 依赖方向：contracts ← harness ← surfaces（contracts 是叶子，谁都不被它依赖回去）。

## 数据流

- 事件通道即协议本身：消费端（CLI logger、TUI）直接订阅 `runtime.events` 的 state/loop 两通道，
  中间不做翻译。将来若再加远程传输，wire 帧应当仍是事件原样透传，而不是另造一套表示。
...
- 这是整条链路的**唯一真相**：不要在任何 surface 重新声明消息或事件的形状。
